...
Entonces el reporte se genera agrupando los datos sin dar error de tiempo de espera.
 ...
 # Escenarios de Leila Ruiz — Control de acceso por rol
  ...
  # Escenarios de Fabrizio
  # Caso Negativo
  Scenario: Intentar generar reporte con rango de fechas inválido
    Dado que el usuario se encuentra en el módulo de reportes
    Cuando ingresa una fecha de inicio posterior a la fecha de fin
    Entonces el sistema debe mostrar un mensaje de error "El rango de fechas ingresado no es válido"
    Y no debe generar ningún reporte

  # Comparativa
  Scenario: Comparar reporte financiero entre dos períodos
    Dado que el usuario se encuentra en el módulo de reportes
    Cuando selecciona dos períodos distintos para comparar
    Entonces el sistema debe mostrar ambos reportes lado a lado con las variaciones porcentuales entre ellos
